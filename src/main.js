...
function launchTransfer(system) {
  ...
  // Har vi redan en live‑process?
  if (isProcessRunning(exeName)) {
    console.log(`[transfer] ${exeName} redan igång`);
    return;
  }

  // Extra säkerhet – stäng ev. gammal referens
  if (transferProc && !transferProc.killed) {
    transferProc.kill();
    transferProc = null;
  }

  transferProc = spawn(exePath, [], {
    detached: true,
    windowsHide: true,
    stdio: 'ignore'
  });
  ...
  // Återstart om den dör oväntat
  transferProc.on('exit', (code, sig) => {
    console.log(`[transfer] dog (code ${code} / sig ${sig})`);
    transferProc = null;
    if (wantedSystem) {
      setTimeout(() => launchTransfer(wantedSystem), 1000);
    }
  });
}
